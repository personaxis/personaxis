...
- **The living engine**: evaluate `personaxis.md` → govern/evolve → compile `PERSONA.md` → memory.
  ...
  [configuration.md](../guides/configuration.md)), never the host's.
...
### Mode 1, Dev companion (augment a coding agent, locally)
...
### Mode 2, Persona runtime in an app (a live persona powering a product feature)
...
Two ways to run the engine in Mode 2:
...
|---|---|---|
| **`@personaxis/core` / `@personaxis/sdk`** | The engine as a library | Mode 2, embed in a Node/TS backend |
| **HTTP `personaxis serve`** | REST + `agents.md` over the engine | Mode 2, drive from any language/process |
