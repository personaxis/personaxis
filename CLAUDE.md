...
This repo is a **pnpm monorepo** (eight lockstep packages) that turns the CLI into a *living, governed persona agent*. The architecture and design docs live in `docs/architecture/` (theorem-to-code map, genesis pipeline, deployment, sandbox, self-evolution, and more).
...
All eight publish at the same lockstep version (read it from any `packages/*/package.json`; it is deliberately not repeated here, because a version written into prose is stale by the next release and this file has already been wrong about it twice); the spec they implement is `spec_version 1.1.0` (1.0.0 validates unchanged, additive; 0.3.0–0.10.0 read-compat via the frozen legacy schema).
