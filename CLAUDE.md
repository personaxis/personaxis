...
All eight publish at the same lockstep version (locally `0.14.0`; latest on npm is `0.12.0`; the next publish lands as `0.15.0`); the spec they implement is `spec_version 1.1.0` (1.0.0 validates unchanged, additive; 0.3.0–0.10.0 read-compat via the frozen legacy schema).
