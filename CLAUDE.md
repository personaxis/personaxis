...
## Before starting work: where the plan is

If a `plan/` folder is present in this checkout, **open `plan/INDEX.md` first**. It is
the entry point after a break: it says which plan is live, which are paused, and
which are history. There is always exactly one live plan, and it is resumed rather
than replaced.

`plan/NEXT_STEPS.md` says where to pick up. It is generated by `pnpm plan:next` and
is never edited by hand, because a handoff written by hand is a handoff that goes
stale.

Three rules the folder enforces, checked by `pnpm plan:check`:

- Every planning document carries `title`, `version`, `date` and `status`, and a
  filename that carries a date must agree with the header.
- A task is not done until its row names **the commit that closed it** and **what
  verified it**. A box ticked by hand fails the check.
- Claims about code carry addresses, and the addresses are checked:
  `pnpm research:check` for the study notes, `pnpm docs:refs` for what the
  documentation says exists.

The folder is gitignored, so a public checkout has none of this and the commands
